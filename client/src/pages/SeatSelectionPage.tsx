...
import { Loader2, Calendar, MapPin, Download, CheckCircle, ArrowLeft } from "lucide-react";
...
export default function SeatSelectionPage() {
  ...
  const [step, setStep] = useState<"seat" | "success">("seat");
  ...
  const downloadTicket = async () => {
    ...
    doc.text(`Seat: Row ${selectedSeat?.row} - Seat ${selectedSeat?.col}`, 20, 135);
          ...
          <Card className="shadow-xl border-primary/10">
            <CardHeader>
              <CardTitle className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-normal text-muted-foreground">Row {selectedSeat?.row || '-'}, Seat {selectedSeat?.col || '-'}</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
               <SeatingGrid 
            ...
            <CardFooter className="flex justify-between">
                ...
                <Button onClick={handleRegister} disabled={!selectedSeat || isPending}>
